...
export function Header({ favoriteCount = 0 }: { favoriteCount?: number }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/70 bg-background/90 backdrop-blur-xl">
      ...
      {menuOpen && <MobileNavigation base={base} onNavigate={() => setMenuOpen(false)} />}
    </header>
  );
}

export function MobileNavigation({ base, onNavigate }: { base: string; onNavigate: () => void }) {
  const { dict } = useLocale();

  return (
    <div className="border-t border-border/70 bg-background px-5 py-4 lg:hidden">
      <nav className="flex flex-col gap-4 text-sm font-medium">
        <Link href={`${base}`} onClick={onNavigate}>{dict.nav.discover}</Link>
        <Link href={`${base}/discover`} onClick={onNavigate}>{dict.nav.browse}</Link>
        <Link href={`${base}#categories`} onClick={onNavigate}>{dict.nav.categories}</Link>
        <Link href={`${base}#artists`} onClick={onNavigate}>{dict.nav.artists}</Link>
      </nav>
    </div>
  );
}
